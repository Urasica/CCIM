...
RETRIEVE_ORIGINAL_TOOL: dict[str, Any] = {
    ...
    "description": (
        ...
        "conversation. This marker represents an evidence span whose original text "
        "has been masked for context efficiency. To read, reason about, quote, or "
        "act on the original evidence, call this tool. "
...
def build_system_hint() -> str:
    ...
    return (
        "Some evidence spans in this conversation have been compressed and replaced "
        "with markers of the form `<<CTX_session:N>>`. Whenever you need to read, "
        "reason about, quote, edit, or take action based on such evidence, you MUST "
        "first call the `retrieve_original` tool with the full context id. If you "
        "need multiple markers, use a single `retrieve_original` call with "
        "`context_ids`. Never invent or paraphrase the masked body."
    )
